                    ...
                    <form id="form-data" method="POST" novalidate>
                        <div class="row mb-4">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="fullname" value="{{$item->fullname}}" required>
                        ...
                        <div class="row mb-4">
                            <div class="form-group col-md-4">
                                ...
                                <select type="text" class="form-select" name="status" required>
                                    <option value="active" @if($item->status == 'active') selected="selected" @endif>Aktif</option>
                                    <option value="inactive" @if($item->status == 'inactive') selected="selected" @endif>Tidak Aktif</option>
                                </select>
                                <div id="feedback-status" class=""></div>
                            </div>
                        </div>
                        <a href="{{route('user-management.user.index')}}" class="btn btn-danger">Kembali</a>
        ...
        <script>
            $(document).ready(function(){
                ...
                form.on('submit', function(e){
                    ...
                    $.ajax({
                        type: 'PUT',
                        url: "{{route('user-management.user.update', $item->id)}}",
                        dataType: 'json',
                        data: formData,
                        beforeSend: function(){
