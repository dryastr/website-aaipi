            ...
            <div class="widget widget-table-one">
                ...
                <h5 class="">{{$title}}</h5>
                    ...
                    <div class="task-action">
                        @can('CMS.icon-footer.create')
                        <a href="{{route('CMS.icon-footer.create')}}" class="btn btn-primary"><i class="fas fa-plus"></i>
                            Tambah</a>
                        @endcan
                    </div>
                </div>
                ...
                $('#table-data').DataTable({
                    ...
                    fnServerData: function (sSource, aoData, fnCallback, oSettings) {
                        ...
                        oSettings.jqXHR = $.ajax({
                            url: "{{route('CMS.icon-footer.view')}}",
                            data: data,
                            success: fnCallback,
                        });
                    },
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row, meta) {
                                ...
                                var linkEdit = "{{route('CMS.icon-footer.edit', ':id')}}".replace(':id',
                                    data.id);
                                var linkDelete = "{{route('CMS.icon-footer.delete', ':id')}}".replace(
                                    ':id', data.id);
                                var html = `
                                        <div class="d-inline-flex">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="+idDropdown+">
                                                @can('CMS.icon-footer.edit')<a class="dropdown-item" href="${linkEdit}">Edit</a>@endcan
                                                @can('CMS.icon-footer.delete')<button class="dropdown-item" onClick="main.confirmDelete('${linkDelete}', '#table-data')">Hapus</button>@endcan
                                            </div>
                                        </div>
                                        `;
                                return html
